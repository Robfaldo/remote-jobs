    ...
    columns do
      column do
        ['indeed', 'linkedin'].each do |job_board|
          panel "#{job_board.upcase} Jobs at each status based on when the job was created" do
          ...
          panel "#{job_board.upcase} Job Previews at each status based on when the job preview was created" do
            table_for ["scraped", "filtered", "evaluated", "total"] do
              column "Status", &:to_s
