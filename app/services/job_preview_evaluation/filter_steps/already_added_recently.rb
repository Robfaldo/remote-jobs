module JobPreviewEvaluation
  module FilterSteps
    class AlreadyAddedRecently < ::JobPreviewEvaluation::Step
      ...
      def call
        filter_job(job_preview, message: "Rejected: job_preview has already been added within 1 week. #{@identical_jobs.uniq.map{|j| { id: j.id, link: j.job_link } }}")
      end

      def can_handle?
        identical_links_already_approved = Job.created_last_week.where(job_link: job_preview.job_link)
