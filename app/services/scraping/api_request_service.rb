module Scraping
  class ApiRequestService
    ...
    def self.call(number_of_attempts:, link:, scraper:, raise_error_after_attempts: true)
      ...
      raise ApiScrapingError.new("404 was returned by #{scraper}. Link: #{link}. Error: #{e}")
    rescue => e
      sleep 5
      retry if (retries += 1) < number_of_attempts

      if raise_error_after_attempts
        raise ApiScrapingError.new("#{scraper} could not scrape after #{retries + 1} attempts. Link: #{link}. Error: #{e}")
      end
    end
  end
end
